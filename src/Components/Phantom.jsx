...
export default function Phantom() {
    ...
    const handleParticipate = async (el) => {
        ...
        if (querySnapshot.empty) {
          ...
          alert("You have now entered into the Airdrop!");
          // reload page
          window.location.reload();
        } else {
            alert('You have already entered the Airdrop.');
        }
    };

    const ConnectButton = () => {
      return (
        <button className="phantom-button" disabled={wallet ? true : false} onClick={(el) => handleConnect(el)}>
  ...
  const handleConnect = async (el) => {
    ...
    if (!provider) {
      window.alert('Phantom wallet is not installed. Install Phantom wallet and try again.');
      window.open('https://phantom.app/', '_blank');
      return;
    }
    try {
  ...
  const toggleParticipate = () =>
    ...
    return (
      <div className="phantom-container" id="phantom">
        ...
        <p>
          ...
          <button
            ...
            onClick={async (el) => {
              ...
              const q = query(collection(db, "wallets"), where("twitter", "==", twitterUsername));
              const querySnapshot = await getDocs(q);
        ...
        <button
          className="phantom-button participate-button"
